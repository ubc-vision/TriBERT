...
class BaseDataset(torchdata.Dataset):
    ...
    def _load_frames(self, paths):
        ...
        for frame in frames: 
            original_img_size.append(frame.size) 
        frames = self.vid_transform(frames)
        return frames, original_img_size
    ...
    def check_video_frames_exists(self, frame_root, index):
        frame_path = os.path.join(frame_root, self.list_sample[index])
        list_of_files = os.listdir(frame_path)
        if len(list_of_files) > 0:
            return index
        else:
            #print("self iteration")
            return self.check_video_frames_exists(frame_root, index+1)
